...
<Controls>
	...
	{:else}
		...
		<Button type="button" onclick={() => setAction(actions.inspect)}
			>Inspect</Button
		>
		<Button type="button" onclick={() => setAction(actions.delete)}
			>Delete</Button
		>
		<Button type="button" onclick={() => setAction(actions.restart)}
			>Restart</Button
		>
	{/if}
...
<style lang="postcss">
	...
	#stats {
		@apply flex flex-col space-y-4 space-x-4 lg:flex-row lg:items-center lg:space-y-0;
	}
